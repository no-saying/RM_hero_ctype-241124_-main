...
void DJIMotorControl()
{
    ...
    { // 减小访存开销,先保存指针引用
        ...
        if ((motor_setting->close_loop_type & SPEED_LOOP) && (motor_setting->outer_loop_type & (ANGLE_LOOP | SPEED_LOOP)))
        {
            ...
            // 更新pid_ref进入下一个环
            // 若该电机处于停止状态,直接将速度目标置零
            if (motor->stop_flag == MOTOR_STOP)
                pid_ref = 0;                    //memset(sender_assignment[group].tx_buff + 2 * num, 0, 16u);
        ...
        if (motor->stop_flag == MOTOR_DEBUG)
        {
            pid_ref = chrip[debug_t++] * 100;
            debug_t %= 20000;
        }
        ...
        if (motor->stop_flag == MOTOR_STOP && abs(motor->measure.speed_aps) < 1)
