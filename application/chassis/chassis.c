...
static Subscriber_t *chassis_power_sub;         // 用于订阅底盘功率的反馈数据
#endif                                              // !ONE_BOARD
static Chassis_Ctrl_Cmd_s chassis_cmd_recv;         // 底盘接收到的控制命令
static Chassis_Power_Data_s chassis_power_recv;     // 底盘功率的控制命令
...
static uint16_t float_to_uintx(float x, float x_min, float x_max, uint8_t bits)
{
...
void ChassisTask()
{
    ...
    case CHASSIS_FOLLOW_GIMBAL_YAW: // 跟随云台,不单独设置pid,以误差角度平方为速度输出
        ...
        chassis_cmd_recv.wz = PIDCalculate(&chassis_follow_to_yaw_pid, chassis_cmd_recv.offset_angle, 0) -1.02 * REAL_WZ_RAT * gimbal_fetch_data.gimbal_imu_data.Gyro[2];
        break;
